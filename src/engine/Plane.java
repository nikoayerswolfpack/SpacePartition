...
public class Plane implements Serializable {
    ...
    public List<Triangle> clipFront(Triangle face) {
        ...
        normalTmp.scale(-1.0);
        return clip(face, normal); /*TODO: IMPLEMENT*/
    }
    ...
    public List<Triangle> clip(Triangle face, Vec3 planeNormal) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if(dot1 >= 0) {
                clippedVertices.add(vertices[1]);

            }
            if (dot1*dot2 < 0) {
                point_i.scale(Math.abs(dot1 / (dot1 - dot2)));
                point_i.add(vertices[i]);
                clippedVertices.add(point_i);
            }
        }
        ...
        if (clippedVertices.size() == 3) {
            triangles.add(
                    t1 = new Triangle(clippedVertices.get(0),
                            clippedVertices.get(1),
                            clippedVertices.get(2)
            ));
            ...
            triangles.add(
                t2 = new Triangle(clippedVertices.get(0),
                        clippedVertices.get(1),
                        clippedVertices.get(2)
            ));
            t1.normal = face.normal;
